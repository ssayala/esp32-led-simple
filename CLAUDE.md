...
Single-file ESP32-S3 firmware (`src/main.cpp`) that drives a DIYables 4-in-1 MAX7219 LED matrix via SPI. It scrolls four content types:
...
- **NVS (ESP32 `Preferences`)** — user config only, set over BLE, survives reboots. Namespaces used: `wifi`, `apikey`, `tickers`, `msgs`, `locs`, `display` (single `mask` byte holding the enabled-category bitmask). **Fetched values (stock quotes, weather readings) are never persisted** — they live in RAM and are re-fetched on boot.
...
`wifi <SSID...> <password>`, `apikey <key>`, `tickers AAPL,MSFT,...`, `messages "a" "b" ...`, `locations "Seattle, WA" 98052 ...`, `mode all | <cat> [<cat> ...]` (cat ∈ {stocks, messages, weather, clock}), `reload`, `reset`.

There's also a SwiftUI iOS app in `ios/` that talks to the same BLE service. See `ios/README.md` for build/run/test details and `ios/LEDTicker/` for the source. The app is a multi-device switcher — one active connection at a time, remembered devices listed on the Device tab with rename/forget swipe actions, MRU auto-connect on launch. Mode changes are made exclusively from the Display tab (multi-category toggles with prereq gating); the Stocks/Weather/Messages tabs read and write only their own data. `Payloads.swift` mirrors `tools/led.py`'s payload formats and `KnownDevice.swift` persists the device list to `UserDefaults`. No firmware-side change is needed when iterating on iOS — the BLE service is the contract.

## Architecture Notes

- **Boot sequence matters** (`setup()`): `initDisplay()` → load user config from NVS (`wifi`, `apikey`, `msgs`, `tickers`, `locs`, `display`) → `buildDeviceName()` → enter `MODE_SETUP(enabledMask)` if any prereq for the saved mask is missing, else `enterContent()` → `showNext()` so the display is live before networking → `connectWifi()` → `initTime()` → `initBLE()` → `triggerFetch()`. `initTime()` is **gated on `WiFi.status() == WL_CONNECTED`** and no-ops otherwise; this avoids starting lwIP's SNTP daemon into a retry loop on no-WiFi boots (the pre-IDF-5 SNTP client accumulates state on failed DNS lookups and wedges the device after 10s of minutes). When WiFi is configured later via BLE, `applyPendingWifi()` calls `initTime()` after the reconnect to kick off NTP.
- **Display mode is a bitmask**: `enabledMask` holds any non-empty subset of `BIT_STOCKS | BIT_MESSAGES | BIT_WEATHER | BIT_CLOCK` (= `MASK_ALL`, `0x0F`). Persisted in NVS namespace `display` under key `mask`. Within `MODE_CONTENT`, `currentBit` tracks which category is scrolling and `advanceCategory()` rotates STOCKS → MESSAGES → WEATHER → CLOCK, skipping bits that are disabled or that have no data. A single-bit mask just never advances. The Mode characteristic accepts `"all"`, a single token, or a comma-separated subset like `"stocks,weather"`; reads return the canonical form (`"all"` when full, otherwise comma-joined). NVS-stored masks from older firmware (max 0x07) are loaded as-is, so existing devices keep their selection and won't auto-enable the new clock bit on upgrade.
- **Static clock fast path**: when `enabledMask == BIT_CLOCK` *and* `timeReady`, `loop()` calls `tickStaticClock()` instead of the normal `display.displayAnimate()` → `showNext()` pump. `tickStaticClock()` formats `"H:MM"` and swaps the `':'` for `' '` on a 500ms cadence (1Hz visual blink) via `displayText(..., PA_PRINT, PA_NO_EFFECT)`, and only redraws when minute or blink phase changes. Before NTP completes, the static branch is skipped and the scroll path renders `"Loading time..."`. In any mixed mode (`BIT_CLOCK` + any other bit) clock scrolls as `"H:MM AM/PM"` via `showNextClock()` like the other categories, and `advanceCategory()` rotates off it immediately since there's only one item per pass.
- **Setup mode** (`MODE_SETUP`): Transient mode shown when prereqs are missing for the requested mask. `setupTargetMask` records the mask to resume into. Entered on boot if the saved mask's prereqs aren't met, when the user writes a mode whose prereqs are missing, or after `cmd=reset`. Cycles two scroll frames (contextual hint + BLE device name like `LED-Ticker-AB12`). The hint targets the first unsatisfied prereq in `setupTargetMask` (WiFi → Finnhub key → weather-needs-WiFi). Clock has the same prereq as weather (WiFi for NTP), so it shares the "Configure WiFi over BLE" hint without a new branch. Exits when prereqs are satisfied (`applyPendingWifi`/`applyPendingApiKey` call `exitSetupIfReady()`, which also persists the new mask), when the user sends an explicit `mode=` write, or after `SETUP_TIMEOUT_MS` (60s) of no BLE activity (falls to `MODE_CONTENT` with the same mask — unmet-prereq bits become "Loading..." hints). Every BLE characteristic `onWrite` resets the activity timer.
...
- **Reset semantics**: `cmd=reset` clears *all* NVS namespaces (including `wifi`, `apikey`, and `display`), zeroes the in-memory `nvsWifiSsid`/`nvsWifiPass`/`nvsApiKey` buffers (otherwise `wifiConfigured()` and `apiKeyConfigured()` keep returning true until reboot), calls `WiFi.disconnect()` to drop the live session, re-seeds tickers and locations from `config.h`, resets `enabledMask` to `MASK_ALL`, and enters `MODE_SETUP(MASK_ALL)`. After reset the device needs WiFi and API key reconfigured over BLE.
- **Display gating** (`showNext()`): the cycle skips category bits whose data isn't ready — stocks only when WiFi+API key are configured *and* `stockCount > 0`; weather only when WiFi is configured *and* `weatherCount > 0`; clock only when `timeReady` (NTP has synced at least once); messages always has fallback text from `config.h`. If no enabled bit has data yet, `showNext()` shows a `"Loading <category>..."` hint for `currentBit`. Mode writes whose prereqs are missing are diverted to `MODE_SETUP` by `applyPendingMode`, but the *mask is still saved* so the device resumes into the user's selection once prereqs are met.
...
- **Main loop** is cooperative: apply pending BLE updates → update status LED → check setup-mode timeout → either `tickStaticClock()` (single-clock fast path) or advance the scroll animation → fetch every `FETCH_INTERVAL_MS` (5 min) if WiFi is up. No `delay()` inside `loop()`.
